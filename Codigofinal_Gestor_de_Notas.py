...
cola_revision = []  # Cola de revisión
...
def eliminar_curso():
    ...
    for i, c in enumerate(cursos):
        if c["nombre"].lower() == nombre:
            confirm = input(f"¿Está seguro que desea eliminar '{c['nombre']}'? (si/no): ").lower()
            if confirm == 'si':
...
# Opcion 8/ Ordenamiento de burbuja por nota
...
# Opcion  9 / Ordenamiento por inserción por nombre
...
def simular_cola():
    ...
    while cola_revision:
        print(f"Revisando: {cola_revision.pop(0)}")
